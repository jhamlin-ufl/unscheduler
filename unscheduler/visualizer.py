        ...
        if event['type'] == 'block':
            ...
            if event.get('spans_midnight', False):
                ...
                if max(s, start_h) < min(24.0, end_h):
                    ax.add_patch(patches.Rectangle((day_index, max(s, start_h)), 1, min(24.0, end_h)-max(s, start_h), facecolor=color, edgecolor='black', linewidth=BLOCK_BORDER_WIDTH, alpha=0.7, zorder=3))
                    ax.text(day_index+0.5, s+0.1, event['event'], ha='center', va='top', color=text_color, fontsize=8, weight='semibold', zorder=5)
                # Morning part
                next_day_index = (day_index + 1) % 7
                if max(0.0, start_h) < min(e, end_h):
                    ax.add_patch(patches.Rectangle((next_day_index, max(0.0, start_h)), 1, min(e, end_h)-max(0.0, start_h), facecolor=color, edgecolor='black', linewidth=BLOCK_BORDER_WIDTH, alpha=0.7, zorder=3))
                    ax.text(next_day_index+0.5, e-0.1, event['event'], ha='center', va='bottom', color=text_color, fontsize=8, weight='semibold', zorder=5)
            else:
                if max(s, start_h) < min(e, end_h):
                    ax.add_patch(patches.Rectangle((day_index, max(s, start_h)), 1, min(e, end_h)-max(s, start_h), facecolor=color, edgecolor='black', linewidth=BLOCK_BORDER_WIDTH, alpha=0.7, zorder=3))
                    ax.text(day_index+0.5, s+(e-s)/2, textwrap.fill(event['event'], 15), ha='center', va='center', color=get_text_color_for_bg(color), fontsize=8, zorder=5)
        
        elif event['type'] == 'trigger':
            t = time_to_float(event['time'])
            if start_h <= t <= end_h:
                trigger_text = f"{format_time_ampm(event['time'])} → {event['event']}"
                ax.text(day_index+0.5, t, trigger_text, ha='center', va='bottom', color=event['color'], fontsize=6, weight='regular', zorder=5)
...
def create_calendar_pdf(events: list, title: str, start_h: int, end_h: int, figsize: tuple):
    ...
    margin = 0.5
    fig.tight_layout(rect=[margin/fig_width, margin/fig_height, 1-(margin/fig_width), 1-(margin/fig_height)], pad=1.0)
